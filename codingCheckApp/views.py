...
def PostDetail(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            file_path = f"codes/{user}/{today}/{today_time}/{file.name}"

            # {project_ROOT}/media/codes/{username}/{today}/{now}/{filename}としてここで直接保存 (models.pyでは定義していない)
            # 例: media/codes/admin/2020-01-01/07.42.44.400874/tmp.py
